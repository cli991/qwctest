...
class LocalTransformersController(BaseLLMController):
    ...
    def __init__(self, model: str = "Qwen/Qwen2.5-3B-Instruct"):
        ...
        if model not in self._shared_models:
            ...
            self._shared_models[model] = (tokenizer, loaded_model)

        self.tokenizer, self.model = self._shared_models[model]
...
class LLMController:
    ...
    def __init__(self, 
        ...
        if backend == "openai":
            ...
            self.llm = LocalTransformersController(model)
        else:
